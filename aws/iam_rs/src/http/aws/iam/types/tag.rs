...
impl validators::NamedValidator for &Tag {
    fn validate(&self, at: &str) -> Result<(), validators::ValidationError> {
        validators::validate_required(self.key(), format!("{at}.{}", "Key").as_str())?;
        validators::validate_named(self.key.as_ref(), format!("{at}.{}", "Key").as_str())?;
        validators::validate_required(self.value(), format!("{at}.{}", "Value").as_str())?;
        validators::validate_named(self.value.as_ref(), format!("{at}.{}", "Value").as_str())?;
        Ok(())
    }
}
