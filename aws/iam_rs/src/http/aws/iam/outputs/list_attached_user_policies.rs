...
pub type LocalListAttachedUserPoliciesOutput = OutputWrapper<ListAttachedUserPoliciesOutput>;

impl From<LocalListAttachedUserPoliciesOutput> for XmlResponse {
    fn from(val: LocalListAttachedUserPoliciesOutput) -> Self {
        let mut out = String::from("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        let mut doc = XmlWriter::new(&mut out);

        let mut response_tag = doc
            .start_el("ListAttachedUserPoliciesResponse")
            .write_ns(constants::xml::IAM_XMLNS, None)
            .finish();

        let mut result_tag = response_tag.start_el("ListAttachedUserPoliciesResult").finish();
