...
pub(crate) struct SimulatePrincipalPolicyRequest {
    ...
    pub(crate) resource_handling_option: Option<
        types::resource_handling_option_type::ResourceHandlingOptionType,
    >,
    ...
    pub(crate) permissions_boundary_policy_input_list: Option<
        Vec<types::policy_document_type::PolicyDocumentType>,
    >,
...
impl SimulatePrincipalPolicyRequest {
    pub(crate) fn action_names(
        &self,
    ) -> Option<&[types::action_name_type::ActionNameType]> {
        self.action_names.as_deref()
    }
    pub(crate) fn caller_arn(&self) -> Option<&str> {
        self.caller_arn.as_deref()
    }
    pub(crate) fn resource_arns(
        &self,
    ) -> Option<&[types::resource_name_type::ResourceNameType]> {
    ...
    pub(crate) fn context_entries(
        &self,
    ) -> Option<&[types::context_entry::ContextEntry]> {
...
impl local_cloud_validate::NamedValidator for &SimulatePrincipalPolicyRequest {
    fn validate(&self, at: &str) -> Result<(), local_cloud_validate::ValidationError> {
        ...
        local_cloud_validate::validate_named(
            self.caller_arn.as_ref(),
            format!("{at}.{}", "CallerArn").as_str(),
        )?;
        ...
        if let Some(permissions_boundary_policy_input_list) = self
            .permissions_boundary_policy_input_list()
        {
            for (id, member) in permissions_boundary_policy_input_list.iter().enumerate()
            {
                local_cloud_validate::validate_named(
                    Some(member),
                    format!("{at}.{}.member.{id}", "PermissionsBoundaryPolicyInputList")
                        .as_str(),
                )?;
            }
        }
