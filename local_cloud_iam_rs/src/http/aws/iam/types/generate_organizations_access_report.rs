...
impl local_cloud_validate::NamedValidator for &GenerateOrganizationsAccessReportRequest {
    fn validate(&self, at: &str) -> Result<(), local_cloud_validate::ValidationError> {
        ...
        local_cloud_validate::validate_required(self.entity_path(), format!("{at}.{}", "EntityPath").as_str())?;
        local_cloud_validate::validate_named(self.entity_path.as_ref(), format!("{at}.{}", "EntityPath").as_str())?;
        Ok(())
    }
}
