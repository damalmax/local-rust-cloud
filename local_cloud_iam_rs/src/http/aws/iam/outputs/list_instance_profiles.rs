...
pub type LocalListInstanceProfilesOutput = OutputWrapper<ListInstanceProfilesOutput>;

impl From<LocalListInstanceProfilesOutput> for XmlResponse {
    fn from(val: LocalListInstanceProfilesOutput) -> Self {
        let mut out = String::from("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        let mut doc = XmlWriter::new(&mut out);

        let mut response_tag = doc
            .start_el("ListInstanceProfilesResponse")
            .write_ns(constants::xml::IAM_XMLNS, None)
            .finish();

        let mut result_tag = response_tag.start_el("ListInstanceProfilesResult").finish();
