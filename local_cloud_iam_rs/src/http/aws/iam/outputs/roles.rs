...
pub(crate) fn write(parent_tag: &mut ScopeWriter, wrapper_tag_name: &str, role: &Role) {
    ...
    write_tag_with_value(&mut wrapper_tag, "Path", Some(role.path()));
    write_tag_with_value(&mut wrapper_tag, "RoleName", Some(role.role_name()));
    write_tag_with_value(&mut wrapper_tag, "RoleId", Some(role.role_id()));
    write_tag_with_value(&mut wrapper_tag, "Arn", Some(role.arn()));
    write_iso8061_datetime_value_tag(&mut wrapper_tag, "CreateDate", Some(role.create_date()));
