...
pub type LocalListAttachedRolePoliciesOutput = OutputWrapper<ListAttachedRolePoliciesOutput>;

impl From<LocalListAttachedRolePoliciesOutput> for XmlResponse {
    fn from(val: LocalListAttachedRolePoliciesOutput) -> Self {
        let mut out = String::from("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
        let mut doc = XmlWriter::new(&mut out);

        let mut response_tag = doc
            .start_el("ListAttachedRolePoliciesResponse")
            .write_ns(constants::xml::IAM_XMLNS, None)
            .finish();

        let mut result_tag = response_tag.start_el("ListAttachedRolePoliciesResult").finish();
