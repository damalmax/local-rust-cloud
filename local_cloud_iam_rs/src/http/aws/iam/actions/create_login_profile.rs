...
impl Action for CreateLoginProfileRequest {
    type Output = OutputWrapper<CreateLoginProfileOutput>;
    ...
    async fn execute(&self, account_id: i64, aws_request_id: &str, db: &LocalDb) -> Result<Self::Output, ApiError> {
        ...
        let output = iam::operations::login_profile::create_login_profile(&ctx, self, db)
            .await
                ...
                OperationError::Service { kind, msg } => {
                    if kind == ApiErrorKind::EntityAlreadyExists {
                        ApiError::new(
                            kind,
                            format!(
                                "IAM login profile for user with name '{}' already exists.",
                                self.user_name().unwrap().trim()
                            ),
                            aws_request_id,
                        )
                    } else {
