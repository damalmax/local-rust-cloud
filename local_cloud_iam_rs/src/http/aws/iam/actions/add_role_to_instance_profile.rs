...
impl Action for AddRoleToInstanceProfileRequest {
    type Output = OutputWrapper<AddRoleToInstanceProfileOutput>;
    ...
    async fn execute(&self, account_id: i64, aws_request_id: &str, db: &LocalDb) -> Result<Self::Output, ApiError> {
        ...
        let output = iam::operations::instance_profile::add_role_to_instance_profile(&ctx, self, db)
            .await
