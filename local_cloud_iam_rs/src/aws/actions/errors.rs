...
use local_cloud_common::request::LocalTag;
    ...
    fn from(value: IamApiError) -> Self {
        ...
        match value.response_type {
            IamErrorResponseType::Single => {
                ...
                local_cloud_xml::write_tag_with_value(&mut error_tag, "Code", Option::Some(value.kind));
                local_cloud_xml::write_tag_with_value(&mut error_tag, "Message", Option::Some(value.message));
                local_cloud_xml::write_request_metadata_tag(&mut error_tag, "ResponseMetadata", "RequestId", value.request_id);
                error_tag.finish();
            }
