...
pub struct EnumWriter<'a, 'b> {
    ...
    derive: Vec<String>,
    fields: Vec<EnumFieldWriter>,
}

impl<'a, 'b> EnumWriter<'a, 'b> {
    pub fn builder(doc: &'a mut String, name: &'b str) -> EnumWriterBuilder<'a, 'b> {
        EnumWriterBuilder {
            doc,
            name,
    ...
    fn write_declaration(&mut self, doc: &mut String) {
        ...
        match self.visibility {
            Visibility::Private => write!(doc, "enum {} {{\n", self.name).unwrap(),
            Visibility::Public => write!(doc, "pub enum {} {{\n", self.name).unwrap(),
            Visibility::PublicCrate => write!(doc, "pub(crate) enum {} {{\n", self.name).unwrap(),
        };
    }
...
#[derive(Debug)]
pub struct EnumWriterBuilder<'a, 'b> {
    doc: &'a mut String,
    name: &'b str,
...
impl<'a, 'b> EnumWriterBuilder<'a, 'b> {
    pub fn with_docs(mut self, docs: Option<String>) -> EnumWriterBuilder<'a, 'b> {
    ...
    pub fn with_visibility(mut self, visibility: Visibility) -> EnumWriterBuilder<'a, 'b> {
        self.visibility = Option::Some(visibility);
        self
    }

    pub fn with_derive(mut self, derive: Vec<impl Into<String>>) -> EnumWriterBuilder<'a, 'b> {
        for item in derive {
    ...
    pub fn with_custom_attributes(mut self, attributes: Vec<impl Into<String>>) -> EnumWriterBuilder<'a, 'b> {
    ...
    pub fn with_fields(mut self, fields: Vec<EnumFieldWriter>) -> EnumWriterBuilder<'a, 'b> {
        for field in fields {
    ...
    pub fn build(self) -> EnumWriter<'a, 'b> {
        EnumWriter {
