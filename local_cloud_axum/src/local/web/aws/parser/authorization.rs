...
use crate::local::web::aws::models::{AuthAlgorithm, Authorization};
use crate::local::web::aws::parser::credential::parse_credential;
use crate::local::web::aws::parser::utils::text;

fn till_whitespace(input: &str) -> IResult<&str, AuthAlgorithm> {
    let (input, algorithm_str) = take_till(|c| c == ' ')(input)?;

    Ok((input, AuthAlgorithm::from_str(algorithm_str)))
}

fn till_comma(input: &str) -> IResult<&str, &str> {
...
fn till_equals(input: &str) -> IResult<&str, &str> {
    let (input, value) = take_till(|c| c == '=')(input)?;
...
pub(crate) fn parse_authorization(input: &str) -> IResult<&str, Authorization> {
    let (input, algorithm) = till_whitespace(input)?;
    ...
    while !input.is_empty() {
        let (input_right, param_key) = till_equals(input.trim())?;
        match param_key {
            "Credential" => {
                if credential_parsed {
                    return fail(input_right);
                }
                let (input_str, credential_str) = till_comma(input_right)?;
            ...
            "SignedHeaders" => {
                ...
                let (input_str, signed_header_names_str) = till_comma(input_right)?;
            ...
            "Signature" => {
                ...
                let (input_str, signature_str) = till_comma(input_right)?;
...
#[cfg(test)]
mod tests {
    use crate::local::web::aws::models::{AuthAlgorithm, Credential};
