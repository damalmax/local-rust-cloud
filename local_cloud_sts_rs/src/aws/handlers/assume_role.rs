...
impl Sts {
    pub async fn assume_role<'a, I: Into<AssumeRoleInput>>(
        ...
        let expiration_seconds =
            i64::try_from(start_time.as_secs()).unwrap() + i64::from(input.duration_seconds().unwrap());
        ...
        let result = AssumeRoleOutput::builder()
            .set_assumed_role_user(Some(assumed_role_user))
            .set_credentials(Some(credentials.as_aws()))
